...
class Runner:
	...
	def run(self):
		...
		for method_choice in self.method_list:
			...
			answer_var_relation = F.normalize(answer_var_relation, p=2, dim=1)  # TODO: concate
	...
	def eval(self):
		...
		for visual_features, boxes, question_features, answers, idx, q_len in tq:
			with torch.no_grad():
				...
				if self.args.joint_test_way == 0:  # way 0
					...
					if args.ZSL == 1:
						zsl_predicts = predicts_ans + self.negtive_mux[:predicts_ans.shape[0], :]
						self.zsl_metrics.update_per_batch(0, answers.data, zsl_predicts.data)  # explain_experiments:  # self.get_entity_id()  # self.explain_experiments(predicts_rel,predicts_fact,predicts_ans,predicts_ans_orig,answers,fact_relation_to_ans,idx)
		...
		if self.get_min_and_max_flag:
			...
			print("self.max:", self.max)  # pdb.set_trace()
	...
	def get_entity_id(self):
		...
		exp_data = os.path.join(self.args.data_root, "fvqa", "exp_data")
		num = "3"
		test_order_data = os.path.join(exp_data, "test_data", "test" + num, "all_qs_dict_release_test_500_inorder.json")
		relation_id_path = os.path.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.relation.500.json")
	...
	def _model_choice(self, args):
		# models api
		fusion_model = getattr(fusion_net, args.fusion_model)(args, self.train_loader.dataset, self.question_word2vec).cuda()
