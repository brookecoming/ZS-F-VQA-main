...
class VisualQA(data.Dataset):
    def __init__(self,
        ...
        self.args = args
        # process question
        # self.args.question_vocab_path = os.path.join(project_root, 'data', 'question.vocab.json') # a joint question vocab across all dataset
    ...
    def _map_gae(self):
        ...
        gae_path = os.path.join(_gae_path, str(self.args.FVQA.gae_node_num) + "_init_" + self.args.FVQA.gae_init + ".pkl")
        ...
        vertices_f = os.path.join(_gae_path, "g_nodes_" + str(self.args.FVQA.gae_node_num) + ".json")
        self.stoi_gae = {}
    ...
    def _map_bert(self):
        ...
        cache_path = os.path.join(self.args.FVQA.bert_path, "map_bert.pt")
        if not os.path.exists(cache_path):
            _bert_path = self.args.FVQA.bert_path

            bert_path = os.path.join(_bert_path, "conceptnet_bert_embeddings.pt")
            ...
            with open(os.path.join(_bert_path, "cn_node_names_for_embeddings.txt"), 'r', encoding='utf-8') as f:
                while 1:
                    line = f.readline()
                    if not line:
                        break
