...
class FVQA(VisualQA):  # ok
    ...
    def _get_cache_path(self, qa_path, file_path, name):
        ...
        if "train" in qa_path:
            cache_filepath = os.path.join(file_path, "fvqa_" + name + "_and_question_train_" +
                                      self.args.method_choice + w2v + "_" + str(self.args.FVQA.max_ans) + ".pt")
        else:
            cache_filepath = os.path.join(file_path, "fvqa_" + name + "_and_question_test_" + self.args.method_choice + w2v + "_" + str(
