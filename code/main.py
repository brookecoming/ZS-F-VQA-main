...
'''
这段代码定义了一个名为Runner的类，它是一个算法的主程序运行器，特别是用于FVQA 任务。
这个类的目的是来准备、训练、评估模型，并执行预测任务。以下是每一步的解释：
'''


class Runner:
    '''
    # 初始化 (__init__ 方法)的步骤如下:
    1 函数初始化时，首先设置了一些基本参数。
	2 配置了日志目录和模型保存目录。
	3 加载了词向量模型，创建了一个指向args.FVQA.common_data_path路径的词向量对象。
	4 创建了两个数据加载器，一个用于训练，一个用于验证。
	5 构建了答案词汇表。
	6 准备了问题的词向量，该步骤中，每个问题中的词都被向量化处理。
	7 获取了融合模型和答案网络。
	8 从ZSL获取了覆盖码。
	9 设置了优化器，该优化器优化融合模型和答复网络的参数。
	10 定义了损失函数，使用了对数似然损失 (LogSoftmax)。
	11 初始化了一些用于记录信息的变量，如最高准确率、最佳周期等。
	12 如果处于测试阶段，就加载模型
    '''

    def __init__(self, args):
        # 初始化函数：设置基本参数、数据、模型和优化器
        # -----1. 准备：数据、模型、损失函数、优化器
        self.log_dir = get_dump_path(args)  # 设置日志目录
        self.model_dir = os.path.join(self.log_dir, 'model')  # 配置模型保存目录。

        self.word2vec = Vector(args.FVQA.common_data_path)  # 加载词向量模型
        # 数据加载：为训练和验证数据集创建数据加载器
        ...
        #  获取融合模型fusion_model 和answer_net
    ...
    def run(self):
        ...
        for epoch in range(self.args.TRAIN.epochs):
            ...
            if epoch % 1 == 0 and epoch > 0:
                ...
                if self.val_metrics.total_loss < (self.correspond_loss - 1) or self.val_metrics.acc_all > (
                        self.max_acc[3] + 0.2):
...
if __name__ == '__main__':
    ...
    #  information output:
    logger.info(
        f"best performance = {runner.max_acc[0]: .2f},{runner.max_acc[1]: .2f},{runner.max_acc[2]: .2f}. best epoch = {runner.best_epoch}, correspond_loss={runner.correspond_loss: .4f}")
    if args.ZSL == 1 and not args.fact_map and not args.relation_map:
        logger.info(
            f" zsl performance = {runner.max_zsl_acc[0]: .2f},{runner.max_zsl_acc[1]: .2f},{runner.max_zsl_acc[2]: .2f}")
    if not cfg.now_test:
