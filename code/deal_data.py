...
class Runner:
	def __init__(self, args):
		self.args = args
		self.path = os.path.join(args.data_root, "data", "FVQA/")
		self.data_path = os.path.join(self.path, "new_dataset_release")
		self.split_path = os.path.join(self.path, "Name_Lists")
		self.exp_data = os.path.join(args.data_root, "fvqa", "exp_data")
		self.e1_list = []
		self.r_list = []
		self.e2_list = []
		self.entity_list = []
	...
	def get_new_all_json(self):
		...
		path = os.path.join(self.data_path, "all_qs_dict_release_combine_all.json")

		if not os.path.exists(path):
			with open(os.path.join(self.data_path, "all_fact_triples_release.json"), "r", encoding='utf8') as ffp:
	...
	def get_new_json(self):
		path = os.path.join(self.data_path, "all_qs_dict_release_combine.json")
		if not os.path.exists(path):
			with open(os.path.join(self.data_path, "all_qs_dict_release_cp.json"), "r") as fp:
				dic = json.load(fp)
				with open(os.path.join(self.data_path, "all_fact_triples_release.json"), "r", encoding='utf8') as ffp:
					dic_all = json.load(ffp)
					# pdb.set_trace()
	...
	def get_entity_filter(self):
		# 把头尾实体筛选一遍，并且储存
		with open(os.path.join(self.data_path, "all_qs_dict_release_combine.json"), 'r') as fp:
			dic = json.load(fp)
			for i in dic.keys():
				for j in [0, 1, 2]:
	...
	def get_all_entity(self):
		path = os.path.join(self.data_path, 'ids_new.data')
		if not os.path.exists(path):
			# 得到所有的头尾实体，并且排序
			entity_list = []
			with open(os.path.join(self.data_path, "FVQA_triple_new_2.txt"), 'r', encoding='utf-8') as f:
				# k = 0
				while 1:
					line = f.readline()
					if not line:
		...
		path = os.path.join(self.data_path, "all_qs_dict_release_combine_filter.json")
	...
	def fusion_answer_and_entity(self):
		...
		# 使用编辑距离
		path = os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion.json")
		if not os.path.exists(path):
			with open(os.path.join(self.data_path, "all_qs_dict_release_combine_filter.json"), 'r') as fp:
				dic = json.load(fp)
				with open(os.path.join(self.data_path, "ans_entity_map.txt"), 'w') as ffp:
					for key in dic.keys():
						strout = "not match: "
						e1 = dic[key]["fact"][0]
						e2 = dic[key]["fact"][2]
				...
				with open(os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion.json"), 'w') as fp:
					json.dump(dic, fp)
	...
	def filter_top500_IQA_pair(self):
		...
		path = os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json")
		if not os.path.exists(path):
			ans_2_id = {}
			with open(os.path.join(self.data_path, "ans.txt"), 'r', encoding='utf-8') as f:
				while 1:
					line = f.readline()
					if not line:
						break
			...
			print(len(ans_2_id))
			with open(os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion.json"), 'r') as fp:
				dic = json.load(fp)
				dic_500 = {key: value for key, value in dic.items() if dic[key]["answer"] in ans_2_id.keys() and ans_2_id[dic[key]["answer"]] <= 500}
	...
	def deal_relation(self):
		path = os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json")
		if not os.path.exists(path):
			with open(os.path.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json"), 'r') as fp:
				dic = json.load(fp)
			relation_set = set()
			for key in dic.keys():
	...
	def split_data(self):
		...
		for i in range(0, 5):
			num = str(i)
			train_name = os.path.join(self.args.FVQA.train_data_path, "train" + num, "all_qs_dict_release_train_500.json")
			test_name = os.path.join(self.args.FVQA.test_data_path, "test" + num, "all_qs_dict_release_test_500.json")
			...
			with open(os.path.join(self.split_path, "train_list_" + num + ".txt"), "r") as f:
				while 1:
					line = f.readline()
					if not line:
						break
			...
			with open(os.path.join(self.split_path, "test_list_" + num + ".txt"), "r") as f:
				while 1:
					line = f.readline()
					if not line:
						break
			...
			# train_name = os.path.join(cfg.FVQA.train_data_path, "train" + num, "all_qs_dict_release_train_500.json")
			# test_name = os.path.join(cfg.FVQA.test_data_path, "test" + num, "all_qs_dict_release_test_500.json")
			ans_train = []
			ans_test = []
			q_train = []
			q_test = []
	...
	def preprocess_fact(self):
		output_format = os.path.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.fact.500.json")
		if not os.path.exists(output_format):
			num = 2
			vqa_train_questions = os.path.join(self.args.FVQA.train_data_path, "train" + str(num), "all_qs_dict_release_train_500.json")
			vqa_val_questions = os.path.join(self.args.FVQA.test_data_path, "test" + str(num), "all_qs_dict_release_test_500.json")
	...
	def preprocess_relation(self):
		output_format = os.path.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.relation.500.json")
		if not os.path.exists(output_format):
			num = 2
			vqa_train_questions = os.path.join(self.args.FVQA.train_data_path, "train" + str(num), "all_qs_dict_release_train_500.json")
			vqa_val_questions = os.path.join(self.args.FVQA.test_data_path, "test" + str(num), "all_qs_dict_release_test_500.json")
	...
	def split_unseen_data(self):
		...
		for i in range(5):
			num = str(i)
			train_name = os.path.join(self.args.FVQA.seen_train_data_path, "train" + num, "all_qs_dict_release_train_500.json")
			test_name = os.path.join(self.args.FVQA.unseen_test_data_path, "test" + num, "all_qs_dict_release_test_500.json")

			if not (os.path.exists(train_name) and os.path.exists(train_name)):
	...
	def get_fact_relation_matrix(self):
		if not os.path.exists(self.args.FVQA.fact_relation_to_ans_path):
			...
			output_format = os.path.join(self.args.FVQA.common_data_path, "fact_relation_dict.data")
			vqa_train_questions = os.path.join(self.args.FVQA.train_data_path, "train2", "all_qs_dict_release_train_500.json")
			vqa_val_questions = os.path.join(self.args.FVQA.test_data_path, "test2", "all_qs_dict_release_test_500.json")
			with open(vqa_train_questions, 'r') as fd:
				qaq1 = json.load(fd)
			with open(vqa_val_questions, 'r') as fd:
				qaq2 = json.load(fd)
	...
	def preprocess_json_in_order(self):
		...
		data_path = os.path.join(self.exp_data, "test_data", "test" + num, "all_qs_dict_release_test_500.json")
		output_format = os.path.join(self.exp_data, "test_data", "test" + num, "all_qs_dict_release_test_500_inorder.json")
	...
	def disjoint_judge(self):
		fact_id_path = os.path.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.fact.500.json")
		answer_id_path = os.path.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.500.json")
	...
	def data_analysis(self, name):
		...
		for num in range(5):
			...
			num = str(num)
			datapath_test = os.path.join(self.exp_data, testpath, "test" + num, "all_qs_dict_release_test_500.json")
			datapath_train = os.path.join(self.exp_data, trainpath, "train" + num, "all_qs_dict_release_train_500.json")
...
if __name__ == '__main__':
	...
	runner.preprocess_json_in_order()  # runner.data_analysis_zsl_and_general()
