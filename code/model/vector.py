...
class Vector(object):
    def __init__(self, cache_path,
        ...
        name = os.path.splitext(os.path.basename(url))[0] + '.txt'  # glove.840B.300d.txt
    ...
    def cache(self, name, cache_path, url=None):
        ...
        if not os.path.isfile(path_pt):
            ...
            if not os.path.exists(path) and url:
                dest = os.path.join(cache_path, os.path.basename(url))
                if not os.path.exists(dest):  # 连这个压缩包都不存在
